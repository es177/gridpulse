...
function RefreshButton() {
  ...
  const ago = lastRefreshed
    ? `${Math.round((Date.now() - lastRefreshed) / 1000)}s ago`
    : null
  ...
  return (
    <button
      onClick={fetchAll}
      disabled={refreshing}
      title={ago ? `Last updated ${ago}` : 'Refresh data'}
      className="flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-mono rounded border transition-all duration-150
        border-blue-500/30 text-blue-400 hover:bg-blue-600/20 hover:border-blue-500/50
